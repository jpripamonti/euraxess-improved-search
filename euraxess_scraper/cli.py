...
from . import config, db, discovery, export as export_mod, indexing, language as language_mod, nli_classifier, search as search_mod, taxonomy, topics
...
app = typer.Typer(help="EURAXESS crawler and hybrid-search CLI")
console = Console()
...
def detect_language_cmd(
    limit: int | None = typer.Option(None, "--limit", min=1, help="Only process the first N jobs"),
    only_missing: bool = typer.Option(
        True,
        "--only-missing/--all",
        help="Only detect language for jobs where it is not yet set",
    ),
    verbose: bool = typer.Option(False, "--verbose", help="Enable debug logs"),
) -> None:
    config.setup_logging(verbose)
    conn = _open_db()
    try:
        rows = db.jobs_for_language_detection(conn, limit=limit, only_missing=only_missing)
        if not rows:
            console.print({"updated": 0, "breakdown": {}})
            return
        ...
        breakdown: dict[str, int] = {}
        for row in rows:
            code = language_mod.detect_language(row["cleaned_text"] or "")
            db.update_job_language(conn, job_id=str(row["job_id"]), language=code)
            breakdown[code] = breakdown.get(code, 0) + 1
            updated += 1
            if updated % 200 == 0:
                conn.commit()
                console.print(f"  ... {updated} processed")
        conn.commit()
        console.print({"updated": updated, "breakdown": breakdown})
    finally:
        conn.close()
...
@app.command("nli-classify-type")
def nli_classify_type(
    limit: int | None = typer.Option(None, "--limit", min=1, help="Only classify the first N jobs"),
    only_unknown: bool = typer.Option(
        True,
        "--only-unknown/--all",
        help="Only classify jobs currently marked as unknown (default). Use --all to re-classify everything.",
    ),
    model_name: str = typer.Option(
        nli_classifier.DEFAULT_NLI_MODEL,
        "--model",
        help="HuggingFace model for zero-shot NLI classification",
    ),
    min_confidence: float = typer.Option(
        nli_classifier.MIN_CONFIDENCE,
        "--min-confidence",
        min=0.0,
        max=1.0,
        help="Minimum NLI score to accept a label (below this → unknown)",
    ),
    verbose: bool = typer.Option(False, "--verbose", help="Enable debug logs"),
) -> None:
    """Classify job types using a zero-shot NLI model (no API key needed).

    Downloads the model on first run (~180 MB, cached in ~/.cache/huggingface/).
    Processes only 'unknown' jobs by default; use --all to reclassify everything.
    """
    config.setup_logging(verbose)
    conn = _open_db()
    try:
        rows = db.jobs_for_nli_classification(conn, limit=limit, only_unknown=only_unknown)
        if not rows:
            console.print({"updated": 0, "note": "No jobs to classify."})
            return

        console.print(f"Loading NLI model '{model_name}' (downloads ~180 MB on first use)…")
        # Warm up the pipeline before the loop so the download message appears once
        nli_classifier._load_pipeline(model_name)
        console.print(f"Model ready. Classifying {len(rows):,} job(s)…")

        updated = 0
        breakdown: dict[str, int] = {}
        for row in rows:
            job_type, score = nli_classifier.classify_job_type_nli(
                title=row["title"],
                researcher_profile=row["researcher_profile"],
                cleaned_text=row["cleaned_text"],
            ...
            db.update_job_type_classification(
                conn,
                job_id=str(row["job_id"]),
                job_type_inferred=job_type,
                job_type_score=score,
            )
            breakdown[job_type] = breakdown.get(job_type, 0) + 1
            updated += 1
            if updated % 100 == 0:
                conn.commit()
                console.print(f"  … {updated:,}/{len(rows):,} classified")
...
@app.command()
def prune(
    delisted: bool = typer.Option(True, "--delisted/--no-delisted", help="Delete jobs marked as delisted"),
    expired: bool = typer.Option(False, "--expired", help="Also delete jobs with a past deadline"),
    verbose: bool = typer.Option(False, "--verbose", help="Enable debug logs"),
) -> None:
    """Hard-delete delisted (and optionally expired) jobs and shrink the database."""
    config.setup_logging(verbose)
    conn = _open_db()
    try:
        deleted = db.delete_pruned_jobs(conn, delisted=delisted, expired=expired)
