...
def _ensure_jobs_schema_migrations(conn: sqlite3.Connection) -> None:
    ...
    if "language" not in columns:
        _safe_add_column(conn, "jobs", "language", "TEXT")
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jobs_language ON jobs(language)")
...
def jobs_for_nli_classification(
    conn: sqlite3.Connection,
    *,
    limit: int | None = None,
    only_unknown: bool = True,
) -> list[sqlite3.Row]:
    """Return jobs to be classified by the NLI model.

    By default returns only jobs with job_type_inferred = 'unknown' / NULL.
    Pass only_unknown=False to re-classify all jobs (e.g. after changing the model).
    """
    clauses = [
        "http_status = 200",
...
def jobs_for_language_detection(
    conn: sqlite3.Connection,
    *,
    limit: int | None = None,
    only_missing: bool = False,
    ...
    if only_missing:
        clauses.append("(language IS NULL OR TRIM(language) = '')")
    where_sql = " AND ".join(clauses)
    sql = f"SELECT job_id, cleaned_text FROM jobs WHERE {where_sql} ORDER BY job_id"
    params: tuple[Any, ...] = ()
    if limit is not None:
        sql += " LIMIT ?"
        params = (limit,)
...
def update_job_language(conn: sqlite3.Connection, *, job_id: str, language: str) -> None:
    conn.execute("UPDATE jobs SET language = ? WHERE job_id = ?", (language, job_id))


def available_languages(conn: sqlite3.Connection) -> list[tuple[str, int]]:
    """Return (language_code, count) pairs for languages present in the DB, ordered by count desc."""
    rows = conn.execute(
        """
...
def facet_counts(
    ...
    *,
    ...
    """Return (value, count) pairs for `field`, applying all active filters EXCEPT the field's own.

    If include_unknown=True, also appends an ("unknown", count) tuple counting rows where the field
    is NULL, empty, or 'unknown'. Useful for job_type_inferred to show unclassified/open-call jobs
    as a separate "unknown" bucket distinct from the real 'other' (Other Profession) value.
    """
    import datetime
    ...
    if field != "job_type_inferred" and job_type and job_type not in {"all", "any", ""}:
        if job_type == "other":
            # "other" = real Other Profession jobs in the DB
            clauses.append("job_type_inferred = 'other'")
        elif job_type == "unknown":
            # "unknown" = unclassified/null/empty jobs (open calls)
            clauses.append(
                "(job_type_inferred IS NULL OR TRIM(job_type_inferred) = '' OR job_type_inferred = 'unknown')"
            )
        else:
    ...
    if field != "topic_domain":
        if include_topics:
            ...
            clauses.append(f"COALESCE(topic_domain, 'other') IN ({placeholders})")
            params.extend(include_topics)
        if exclude_topics:
            placeholders = ",".join("?" * len(exclude_topics))
            clauses.append(f"COALESCE(topic_domain, 'other') NOT IN ({placeholders})")
    ...
    where_sql = " AND ".join(clauses)
    sql = f"""
        SELECT COALESCE({field}, 'unknown') AS val, COUNT(*) AS c
        FROM jobs
        WHERE {where_sql}
    ...
    """
    rows = conn.execute(sql, params).fetchall()
    results = [(row["val"], int(row["c"])) for row in rows]
    ...
    if include_unknown:
        unknown_sql = f"""
            ...
            FROM jobs
            WHERE {where_sql}
              AND ({field} IS NULL OR TRIM({field}) = '' OR {field} = 'unknown')
        """
        unknown_count = int(conn.execute(unknown_sql, params).fetchone()["c"])
        if unknown_count > 0:
            results.append(("unknown", unknown_count))
...
def delete_pruned_jobs(
    ...
    *,
    ...
    """Hard-delete jobs and their queue entries, then VACUUM.

    delisted=True  → delete jobs where delisted_at IS NOT NULL
    expired=True   → also delete jobs with a past deadline (requires now_iso)
    ...
    clauses: list[str] = []
    if delisted:
        clauses.append("delisted_at IS NOT NULL")
    if expired:
    ...
    # Remove queue entries first (no FK cascade in SQLite by default)
    conn.execute(
        f"DELETE FROM queue WHERE job_id IN (SELECT job_id FROM jobs WHERE {where_sql})",
        params,
    )
    cur = conn.execute(f"DELETE FROM jobs WHERE {where_sql}", params)
