...
def _fake_encode_texts(texts, _model_name):
    ...
    for text in texts:
        t = str(text).lower()
        if any(token in t for token in ["engineering", "machine learning", "computer science", "informatique", "apprentissage automatique"]):
...
def test_multilingual_examples_map_to_expected_buckets(monkeypatch):
    ...
    assert stem.topic_domain == "engineering_technology"
